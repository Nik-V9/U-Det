...
def main(args):
    ...
    if args.direct_test:
        mask_list = []
        root_path = '/content/drive/My Drive/LNDB/Dataset'
        for ext in ('*.npy'):
        ...
        for i in range(len(mask_list)):
            name = basename(mask_list[i])
            name = name.replace('masks_','')
            t_list.append(name)
    ...
    image = np.load(join(args.data_root_dir, 'imgs', 'images_'+train_list[0])).T
...
if __name__ == '__main__':
    ...
    parser.add_argument('--direct_test', type=int, default=0, choices=[0,1],
                        help='0 or 1 for direct testing')
